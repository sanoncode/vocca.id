  ...
  return (
    <header className="p-4 flex items-center justify-between border-b bg-background text-foreground">
                ...
                (avatar, index) =>
                  isRoomHost(avatar.id) && (
                    ...
                    >
                      ...
                      {avatar.lang_code && (
                        <span className="absolute -bottom-1 -right-1 min-w-[18px] h-[18px] flex items-center justify-center text-[8px] font-bold leading-none bg-background text-zinc-600 dark:text-zinc-300 rounded-full shadow-sm border border-zinc-200 dark:border-zinc-800 px-0.5 select-none">
                          {languageMap[avatar.lang_code].badge}
                        </span>
                      )}
                    </div>
                  ),
            ...
            <div className="flex -space-x-2 hover:space-x-1 transition-all duration-300 ease-in-out">
              {roomData.avatars?.map((avatar, index) => (
                ...
                >
                  ...
                  {/* Language Badge di Pojok Kanan Bawah */}
                  {avatar.lang_code && (
                    <span className="absolute -bottom-1 -right-1 min-w-[18px] h-[18px] flex items-center justify-center text-[8px] font-bold leading-none bg-background text-zinc-600 dark:text-zinc-300 rounded-full shadow-sm border border-zinc-200 dark:border-zinc-800 px-0.5 select-none">
                      {languageMap[avatar.lang_code].badge}
                    </span>
      ...
      {isRoomHost(currentUser?.id) ? (
        ...
        <ChatLeaveRoomButton roomId={roomId} handleLeave={handleLeave} />
      )}
    </header>
  );
};
